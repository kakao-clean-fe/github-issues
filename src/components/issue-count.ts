import { findParentAndRenderInnerHtml, findElement, addClickEventListener } from '~/utils/dom';
  ...
  return [
    {
      ...
      eventHandler: () => pipe(filterOpenedIssues, (issues: Issue[]) => IssueList({ issues }))(issues)
    },
    {
      selector: ISSUE_CLOSE_COUNT_SELECTOR,
      eventHandler: () => pipe(filterClosedIssues, (issues: Issue[]) => IssueList({ issues }))(issues)
    }
  ];
};
